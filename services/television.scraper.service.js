...
async function fetchImageUrls() {
  try {
    ...
    const showElements = dom.window.document.querySelectorAll(
    ...
    return Array.from(showElements).map((show) => {
      ...
      const showId = showLinkElement
        ? showLinkElement.href.match(/\/show\/(\d+)/)?.[1]
        : null;
  ...
  try {
    ...
    const results = htmls.map((html, index) => {
      ...
      const titleElement = dom.window.document.querySelector(
        ".col-lg-12.show-title"
      );
      const descriptionElement = dom.window.document.querySelector(
        ".col-lg-12.show-descr p"
      );
      const videoLinkElements =
        dom.window.document.querySelectorAll(".popup-youtube");

      const postTitleElement = dom.window.document.querySelector(
        ".single-vpost-content .single-vpost-title"
      );
      ...
      const imageUrlObj = imageUrls.find(
        (img) => img.showId === (index + 1).toString()
      );
